...
<template>
  <main class="page page--workbench">
    ...
    <div class="page-intro">
      <div>
        <p class="eyebrow">PROVEN TODAY / OPERATOR WORKSPACE</p>
        <h1>Physical evidence → canonical custody</h1>
        <p>Run the hackathon proof end to end: resolve one animal, request Station evidence, authorize with the current custodian wallet, then wait for finalized Solana state.</p>
      </div>
    </div>
        ...
        <AppCard>
          <header class="app-card__header">
            <div>
              ...
              <p class="app-card__description">Availability comes from the existing protocol prerequisites; disabled actions explain what is missing.</p>
            </div>
          </header>
          ...
          <div class="action-grid">
            <article class="action-card" data-kind="origin">
              ...
              <AppButton :disabled="busy || !canOrigin" :busy="busy && canOrigin" @click="runAction('ORIGIN')">Origin</AppButton>
            </article>
            ...
            <article class="action-card" data-kind="transfer">
              ...
              <AppButton :disabled="busy || !canTransfer" :busy="busy && canTransfer" @click="runAction('TRANSFER')">Transfer</AppButton>
            </article>
            ...
            <article class="action-card" data-kind="reidentify">
              ...
              <AppButton :disabled="busy || !canReidentify" :busy="busy && canReidentify" @click="runAction('REIDENTIFY')">Reidentify</AppButton>
            </article>
          </div>

          <div v-if="canRunStaleCustodianAttempt" class="identity-resolution">
            <p>Connected wallet is not the current custodian. The explicit stale-authority check creates no capture, evidence or transaction.</p>
            <AppButton variant="danger" :disabled="busy" @click="runInvalidOldCustodianAttempt">Run stale-custodian attempt</AppButton>
          </div>
        </AppCard>
      </div>
        ...
        <AppCard>
          <header class="app-card__header">
            <div>
              ...
              <p class="app-card__description">The visual recovery identifier resolves digital identity; it never grants custody authority.</p>
            </div>
          </header>
          ...
          <div class="control">
            ...
            <input
              ...
              :disabled="busy"
            />
            <p class="control__help">Use the independent recovery identifier when the current RFID is unavailable.</p>
          </div>

          <div class="control-row control-row--spaced">
            <AppButton :disabled="busy || !visualRecoveryId" :busy="busy" @click="createAnimal">Create Animal</AppButton>
            <AppButton variant="secondary" :disabled="busy || !visualRecoveryId" :busy="busy" @click="recoverAnimal">Find by visual recovery ID</AppButton>
          </div>

          <div class="identity-resolution">
            <p><strong>Physical identity continuity:</strong> {{ animal ? 'RESOLVED' : 'UNRESOLVED' }}</p>
            <span class="status-badge" :data-tone="animal ? 'proof' : undefined">{{ animal ? 'RESOLVED' : 'UNRESOLVED' }}</span>
          </div>

          <p v-if="!animal" class="control__help control__help--spaced">
            Provide the independent visual recovery identifier, or resolve the current RFID through an authoritative RFID lookup, before starting a transition. Lastro does not guess identity when both physical identifiers are unavailable.
          </p>
        </AppCard>
        ...
        <AppCard>
          <div class="operation-status">
            ...
            <p
              ...
              :data-tone="operationStage === 'ERROR' ? 'danger' : operationStage === 'FINALIZED' ? 'success' : undefined"
            ...
            <RouterLink v-if="animal?.eventSequence" class="button" :to="'/verify/' + animal.animalId">
...
import { clearPendingOperation, readPendingOperation, writePendingOperation, type PendingOperation } from '../demo/pendingOperation'
import { decodeStationEvent } from '../protocol/stationEvent'
import type { EvidencePackage } from '../protocol/evidence'
import { submitLastroTransaction } from '../solana/transaction'
import { availableWalletChoices, connectFirstAvailableWallet, connectWalletByName, currentWalletAddress, walletAddressToCustodianHex, type WalletChoice } from '../solana/wallet'
...
type OperationStage = 'IDLE' | 'WAITING_STATION' | 'EVIDENCE_SIGNED' | 'WALLET_AUTHORIZATION' | 'SUBMITTED' | 'FINALIZED' | 'ERROR'
...
const canOrigin = computed(() => animal.value !== null
...
const canTransfer = computed(() => animal.value !== null
  && animal.value.eventSequence > 0
  && connectedCustodian.value !== null
  && animal.value.currentCustodian === connectedCustodian.value
  && nextCustodianAddress.value.length > 0)
const canReidentify = computed(() => animal.value !== null
  && animal.value.eventSequence > 0
  && connectedCustodian.value !== null
  && animal.value.currentCustodian === connectedCustodian.value)
const canRunStaleCustodianAttempt = computed(() => animal.value !== null
  && animal.value.eventSequence > 0
  && animal.value.currentCustodian !== null
  && connectedCustodian.value !== null
  && animal.value.currentCustodian !== connectedCustodian.value)
...
const originAvailability = computed(() => {
  ...
  if (animal.value.eventSequence !== 0 || animal.value.currentCustodian !== null || animal.value.currentRfidHash !== null) {
...
const transferAvailability = computed(() => {
  ...
  if (animal.value.currentCustodian !== connectedCustodian.value) return 'Connected wallet is not the current custodian.'
...
const operationTitle = computed(() => {
  switch (operationStage.value) {
    case 'WAITING_STATION': return 'Waiting for physical evidence'
    case 'EVIDENCE_SIGNED': return 'Station evidence signed'
    case 'WALLET_AUTHORIZATION': return 'Wallet authorization required'
    case 'SUBMITTED': return 'Waiting for Solana finality'
    case 'FINALIZED': return 'Canonical state finalized'
    case 'ERROR': return 'Operation needs attention'
    default: return 'Idle'
  }
})
...
const operationLabel = computed(() => {
  switch (operationStage.value) {
    case 'WAITING_STATION': return 'WAITING'
    case 'EVIDENCE_SIGNED': return 'SIGNED'
    case 'WALLET_AUTHORIZATION': return 'WAITING'
...
const operationTone = computed<OperationTone>(() => {
  switch (operationStage.value) {
    case 'EVIDENCE_SIGNED': return 'proof'
    case 'WALLET_AUTHORIZATION': return 'primary'
    case 'SUBMITTED': return 'chain'
...
async function connectWallet(): Promise<void> {
  await guarded(async () => {
    ...
    refreshWalletChoices()
    const connectedChoice = walletChoices.value.find((choice) => choice.address === walletAddress.value)
...
async function runAction(action: CaptureAction): Promise<void> {
  await guarded(async () => {
    ...
    if (!connectedAddress) throw new Error('Connect the custodian wallet before starting a protected action')
    walletAddress.value = connectedAddress

    let nextCustodian: Hex32 | null = null
    if (action === 'ORIGIN') {
      nextCustodian = walletAddressToCustodianHex(connectedAddress)
    } else if (action === 'TRANSFER') {
      nextCustodian = walletAddressToCustodianHex(nextCustodianAddress.value)
    }

    if (action !== 'ORIGIN' && selected.currentCustodian !== walletAddressToCustodianHex(connectedAddress)) {
      throw new Error('Connected wallet is not the current custodian; capture was not created')
    }

    const capture = await api.createCapture(action, selected.animalId, nextCustodian)
    ...
    operationStage.value = capture.status === 'EVIDENCE_ACCEPTED' ? 'EVIDENCE_SIGNED' : 'WAITING_STATION'
    message.value = capture.status === 'EVIDENCE_ACCEPTED'
...
async function completeAcceptedCapture(accepted: Capture, pending: PendingOperation): Promise<void> {
  if (!accepted.eventHash) throw new Error('Accepted capture did not expose its immutable eventHash')
  ...
  if (!txSignature) {
    ...
    txSignature = await submitLastroTransaction(transactionData)
  ...
  if (accepted.eventStatus !== 'SUBMITTED' && accepted.eventStatus !== 'FINALIZED') {
    message.value = 'Transaction broadcast. Waiting for exact transaction verification at confirmed commitment.'
    await waitForSubmitted(accepted.eventHash, txSignature)
  }
...
async function runInvalidOldCustodianAttempt(): Promise<void> {
  await guarded(async () => {
    const selected = animal.value
    const connectedAddress = currentWalletAddress()
    if (!selected?.currentCustodian || !connectedAddress) throw new Error('An originated animal and connected wallet are required')
    walletAddress.value = connectedAddress
    const connectedCustodian = walletAddressToCustodianHex(connectedAddress)
    if (connectedCustodian === selected.currentCustodian) {
      throw new Error('Connected wallet is the current custodian. Switch to the previous custodian wallet to run the stale-authority check.')
    }
    message.value = 'REJECTED: connected wallet is not the current custodian. No capture, evidence, or transaction was created.'
  })
}

async function waitForSubmitted(eventHash: Hex32, txSignature: string): Promise<void> {
  for (let attempt = 0; attempt < SUBMISSION_POLL_ATTEMPTS; attempt += 1) {
    try {
      const submission = await api.submit(eventHash, txSignature)
      if (submission.txSignature !== txSignature) throw new Error('API returned a different submitted transaction signature')
      if (submission.status !== 'SUBMITTED' && submission.status !== 'FINALIZED') throw new Error('API returned an invalid transaction submission status')
      return
    } catch (error) {
      if (!isSubmissionPending(error) || attempt === SUBMISSION_POLL_ATTEMPTS - 1) throw error
...
function isSubmissionPending(error: unknown): boolean {
  return conflictMessage(error) === 'transaction is not a confirmed exact Lastro transaction for this event'
}

function isFinalityPending(error: unknown): boolean {
  return conflictMessage(error) === 'transaction is not a finalized exact Lastro transaction for this event'
}
...
async function activateAnimal(projection: AnimalProjection): Promise<void> {
  ...
  timeline.value = projection.eventSequence === 0
    ? []
...
function timelineFromEvidence(pkg: EvidencePackage): Array<{ sequence: number; label: string }> {
  return pkg.events.map((evidence) => {
    const raw = Uint8Array.from(atob(evidence.eventBytesBase64), (character) => character.charCodeAt(0))
    const event = decodeStationEvent(raw)
    const sequence = Number(event.eventSequence)
    if (!Number.isSafeInteger(sequence)) throw new Error('EvidencePackage event sequence exceeds browser safe integer range')
    if (event.action === 1) return { sequence, label: `#${sequence} ORIGIN — custodian ${hex(event.toCustodian)}` }
    if (event.action === 2) return { sequence, label: `#${sequence} TRANSFER — custodian ${hex(event.toCustodian)}` }
    return { sequence, label: `#${sequence} REIDENTIFY — RFID ${hex(event.newRfidHash)}, revision ${event.identityRevision}` }
  })
}
...
async function restoreRememberedSession(): Promise<void> {
  ...
  await guarded(async () => {
    ...
    if (!txSignature) {
      operationStage.value = 'WALLET_AUTHORIZATION'
      message.value = 'Recovered accepted Station evidence. Connect the required wallet and repeat the same action to authorize it; no new RFID observation will be created.'
      return
    }
    ...
    if (accepted.eventStatus !== 'SUBMITTED' && accepted.eventStatus !== 'FINALIZED') {
      operationStage.value = 'WALLET_AUTHORIZATION'
      message.value = 'Recovered a broadcast transaction. Waiting for confirmed transaction verification.'
      await waitForSubmitted(accepted.eventHash, txSignature)
    }
    operationStage.value = 'SUBMITTED'
