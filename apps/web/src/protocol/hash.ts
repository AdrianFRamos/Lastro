...
import { CANONICAL_RFID_LENGTH, RFID_DOMAIN, STATION_DOMAIN, STATION_EVENT_LENGTH } from './constants'
...
export const eventHash = async (eventBytes: Uint8Array) => {
  if (eventBytes.length !== STATION_EVENT_LENGTH) throw new Error('StationEvent must be exactly 276 bytes')
  return sha256(eventBytes)
}

export const rfidHash = async (canonicalRfid: Uint8Array) => {
  if (canonicalRfid.length !== CANONICAL_RFID_LENGTH) throw new Error('canonical RFID must be exactly 8 bytes')
  return sha256(concat(RFID_DOMAIN, canonicalRfid))
}

export const stationId = async (compressedP256Pubkey: Uint8Array) => {
  if (compressedP256Pubkey.length !== 33 || (compressedP256Pubkey[0] !== 0x02 && compressedP256Pubkey[0] !== 0x03)) {
