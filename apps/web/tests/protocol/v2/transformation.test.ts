...
const inputs: V2LineageLeaf[] = [
  { assetIdHex: '11'.repeat(32), role: V2LineageRole.Input, position: 0, quantity: 1n, weightGrams: 600n },
  { assetIdHex: '12'.repeat(32), role: V2LineageRole.Input, position: 1, quantity: 1n, weightGrams: 400n },
]

const outputs: V2LineageLeaf[] = [
  { assetIdHex: '21'.repeat(32), role: V2LineageRole.Output, position: 0, quantity: 1n, weightGrams: 800n },
  { assetIdHex: '22'.repeat(32), role: V2LineageRole.Byproduct, position: 1, quantity: 1n, weightGrams: 150n },
]
...
describe('Lastro protocol v2 transformation canonicalization', () => {
  it('matches the checked-in roots and fixed manifest length', async () => {
    ...
    await expect(v2TransformationManifestHash(manifest)).resolves.toBe('977bc957839b450db6c2a43de720ba540393dee37678107fef525e740da61fbc')
  })

  it('rejects duplicate positions and mass outside tolerance', async () => {
    await expect(v2MerkleRoot([inputs[0], { ...inputs[1], position: 0 }])).rejects.toThrow('positions')
    const inputRootHex = await v2MerkleRoot(inputs)
    const outputRootHex = await v2MerkleRoot(outputs)
    await expect(v2TransformationManifestHash({
      transformationIdHex: '01'.repeat(32),
      facilityIdHex: '02'.repeat(32),
      transformationType: 1,
      inputRootHex,
