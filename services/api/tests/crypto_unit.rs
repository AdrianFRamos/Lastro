...
fn valid_origin_evidence_preserves_exact_station_bytes() {
    ...
    let key: [u8; 33] = hex::decode(&valid_request().station_pubkey_hex).unwrap().try_into().unwrap();
    let verified = verify_agent_evidence(&valid_request(), &key).unwrap();
    assert_eq!(verified.event_bytes.as_slice(), fixture("origin.bin"));
    assert_eq!(hex::encode(verified.event_hash), "5845dc20fd6b266ec98399f0aa93c736ec9aa778bf038af5291e5df81334b531");
}
...
fn unregistered_station_key_is_rejected() {
    ...
    let other: [u8; 33] = hex::decode("037cf27b188d034f7e8a52380304b51ac3c08969e277f21b35a60b48fc47669978").unwrap().try_into().unwrap();
