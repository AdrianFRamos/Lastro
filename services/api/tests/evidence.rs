...
use lastro_protocol::{evidence::EvidencePackage, StationEvent};
use serde_json::Value;
use tower::ServiceExt;
use uuid::Uuid;

use common::{app_state, TestDb, TestRpc, DEPLOYMENT_ID, STATION_PUBKEY};
...
async fn fetch_package(db: &TestDb) -> (StatusCode, Value) {
    let response = app(db)
        .oneshot(
            Request::builder()
                .uri(format!("/api/animals/{}/evidence-package", hex::encode(ANIMAL_ID)))
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn evidence_package_orders_events_by_sequence() {
    ...
    let sequences: Vec<u64> = events
        .iter()
        .map(|item| {
            let raw = BASE64.decode(item["eventBytesBase64"].as_str().unwrap()).unwrap();
...
async fn evidence_package_contains_original_event_bytes() {
    ...
    for (item, fixture_name) in body["events"].as_array().unwrap().iter().zip([
        ...
        let decoded = BASE64.decode(item["eventBytesBase64"].as_str().unwrap()).unwrap();
...
async fn evidence_package_contains_observed_rfid_pubkey_signature_tx() {
    ...
    for (item, (rfid, signature, tx_signature)) in body["events"].as_array().unwrap().iter().zip(expected) {
        assert_eq!(item["observedRfidHex"].as_str(), Some(hex::encode(rfid).as_str()));
        assert_eq!(item["stationPubkeyHex"].as_str(), Some(hex::encode(STATION_PUBKEY).as_str()));
...
async fn evidence_package_excludes_backend_valid_boolean() {
    ...
    assert_eq!(object.keys().cloned().collect::<std::collections::BTreeSet<_>>(), [
        "animalId".to_string(),
...
async fn evidence_package_matches_schema_contract_and_strict_serde() {
    ...
    assert_eq!(body["deploymentId"].as_str(), Some(hex::encode(DEPLOYMENT_ID).as_str()));
    assert_eq!(body["animalId"].as_str(), Some(hex::encode(ANIMAL_ID).as_str()));
    ...
    for item in body["events"].as_array().unwrap() {
        let keys: std::collections::BTreeSet<_> = item.as_object().unwrap().keys().map(String::as_str).collect();
        assert_eq!(keys, [
            "eventBytesBase64",
            "observedRfidHex",
            "stationPubkeyHex",
        ...
        assert_eq!(BASE64.decode(item["eventBytesBase64"].as_str().unwrap()).unwrap().len(), 276);
    ...
    let package: EvidencePackage = serde_json::from_value(body.clone()).expect("strict EvidencePackage decode");
    assert_eq!(serde_json::to_value(&package).unwrap(), body);
    package.validate_off_chain_chain().expect("real API package validates off-chain");

    let mut with_extra = body;
    with_extra.as_object_mut().unwrap().insert("valid".into(), Value::Bool(true));
