...
use lastro_protocol::{crypto::derive_station_id, ids::{AnimalId, DeploymentId, EventHash, RfidHash}};
use reqwest::Client;
...
use crate::{
    ...
    solana::transaction_builder::{animal_state_address, parse_program_id, protocol_config_address, rfid_binding_address},
};
...
pub trait SolanaRpc: Send + Sync {
    async fn protocol_station_pubkey(&self, deployment_id: DeploymentId) -> Result<[u8; 33], ApiError>;
    async fn animal_state(&self, deployment_id: DeploymentId, animal_id: AnimalId) -> Result<Option<CanonicalAnimalState>, ApiError>;
    async fn rfid_binding(&self, deployment_id: DeploymentId, rfid_hash: RfidHash) -> Result<Option<CanonicalRfidBinding>, ApiError>;
...
impl HttpSolanaRpc {
    pub fn new(url: String, lastro_program_id: String) -> Self {
        Self { url, lastro_program_id, client: Client::new() }
    }
    ...
    async fn call(&self, method: &str, params: Value) -> Result<Value, ApiError> {
        ...
        if !response.status().is_success() {
            return Err(ApiError::Unavailable("Solana RPC returned an HTTP error".into()));
        }
    ...
    async fn account_data(&self, address: &Pubkey) -> Result<Option<Vec<u8>>, ApiError> {
        ...
        let value = result
            .get("value")
            .ok_or_else(|| ApiError::Unavailable("Solana account response is missing value".into()))?;
        if value.is_null() {
            return Ok(None);
        }
        ...
        if owner != self.lastro_program_id {
            return Err(ApiError::Conflict("canonical account is not owned by the configured Lastro program".into()));
        }
        if value.get("executable").and_then(Value::as_bool) != Some(false) {
            return Err(ApiError::Conflict("canonical Lastro state account must not be executable".into()));
        }
        let data = value.get("data").and_then(Value::as_array)
            .ok_or_else(|| ApiError::Unavailable("Solana account data is not base64 encoded".into()))?;
        if data.len() != 2 || data[1].as_str() != Some("base64") {
            return Err(ApiError::Unavailable("Solana account data encoding is not base64".into()));
        }
        let encoded = data[0].as_str()
            .ok_or_else(|| ApiError::Unavailable("Solana account data payload is invalid".into()))?;
        let decoded = BASE64_STANDARD.decode(encoded)
            .map_err(|_| ApiError::Unavailable("Solana account data contains invalid base64".into()))?;
        if BASE64_STANDARD.encode(&decoded) != encoded {
            return Err(ApiError::Unavailable("Solana account data base64 is not canonical".into()));
        }
        Ok(Some(decoded))
    }
}
...
impl SolanaRpc for HttpSolanaRpc {
    async fn protocol_station_pubkey(&self, deployment_id: DeploymentId) -> Result<[u8; 33], ApiError> {
        ...
        let data = self.account_data(&address).await?
    ...
    async fn animal_state(&self, deployment_id: DeploymentId, animal_id: AnimalId) -> Result<Option<CanonicalAnimalState>, ApiError> {
        let program_id = parse_program_id(&self.lastro_program_id)?;
        let (address, expected_bump) = animal_state_address(&program_id, &deployment_id, &animal_id);
        let Some(data) = self.account_data(&address).await? else { return Ok(None); };
        decode_animal_state(&data, animal_id, expected_bump).map(Some)
    }

    async fn rfid_binding(&self, deployment_id: DeploymentId, rfid_hash: RfidHash) -> Result<Option<CanonicalRfidBinding>, ApiError> {
        let program_id = parse_program_id(&self.lastro_program_id)?;
        let (address, expected_bump) = rfid_binding_address(&program_id, &deployment_id, &rfid_hash);
        let Some(data) = self.account_data(&address).await? else { return Ok(None); };
    ...
    async fn transaction_matches_confirmed(
        ...
        self.transaction_matches_at_commitment(signature, expected, "confirmed").await
    }
    ...
    async fn transaction_matches(
        ...
        self.transaction_matches_at_commitment(signature, expected, "finalized").await
    }
}
...
fn expect_layout<'a>(data: &'a [u8], account: &str, len: usize) -> Result<&'a [u8], ApiError> {
    if data.len() != len || data[..8] != anchor_discriminator(account) {
        return Err(ApiError::Conflict(format!("canonical {account} account has an invalid layout")));
    }
    Ok(data)
}

fn decode_protocol_config(data: &[u8], deployment_id: DeploymentId, expected_bump: u8) -> Result<[u8; 33], ApiError> {
    ...
    if stored_deployment != deployment_id || data[105] != expected_bump {
        return Err(ApiError::Conflict("canonical ProtocolConfig does not match its PDA seeds".into()));
    }
    let station_key: [u8; 33] = data[72..105].try_into().map_err(|_| ApiError::Internal)?;
    derive_station_id(&station_key)
        .map_err(|_| ApiError::Conflict("canonical ProtocolConfig contains an invalid Station public key".into()))?;
    Ok(station_key)
}

fn decode_animal_state(data: &[u8], expected_animal_id: AnimalId, expected_bump: u8) -> Result<CanonicalAnimalState, ApiError> {
    ...
    if animal_id != expected_animal_id || data[148] != expected_bump {
        return Err(ApiError::Conflict("canonical AnimalState does not match its PDA seeds".into()));
    }
    Ok(CanonicalAnimalState {
        ...
        identity_revision: u32::from_le_bytes(data[104..108].try_into().map_err(|_| ApiError::Internal)?),
        event_sequence: u64::from_le_bytes(data[108..116].try_into().map_err(|_| ApiError::Internal)?),
        last_event_hash: data[116..148].try_into().map_err(|_| ApiError::Internal)?,
    })
}

fn decode_rfid_binding(data: &[u8], expected_rfid_hash: RfidHash, expected_bump: u8) -> Result<CanonicalRfidBinding, ApiError> {
    ...
    if rfid_hash != expected_rfid_hash || data[73] != expected_bump {
        return Err(ApiError::Conflict("canonical RfidBinding does not match its PDA seeds".into()));
    }
    let status = match data[72] {
        1 => BindingStatus::Active,
        2 => BindingStatus::Retired,
        _ => return Err(ApiError::Conflict("canonical RfidBinding contains an invalid status".into())),
    };
...
fn transaction_matches_json(
    ...
    if result.get("version").is_some_and(|version| !version.is_null() && version.as_str() != Some("legacy")) {
        return Ok(false);
    }
    let meta = result.get("meta").and_then(Value::as_object)
        .ok_or_else(|| ApiError::Unavailable("Solana transaction is missing metadata".into()))?;
    if meta.get("err").is_none_or(|err| !err.is_null()) {
        return Ok(false);
    }

    let transaction = result.get("transaction").and_then(Value::as_object)
        .ok_or_else(|| ApiError::Unavailable("Solana transaction payload is invalid".into()))?;
    let signatures = transaction.get("signatures").and_then(Value::as_array)
    ...
    let message = transaction.get("message").and_then(Value::as_object)
        .ok_or_else(|| ApiError::Unavailable("Solana transaction message is invalid".into()))?;
    if message.get("addressTableLookups").is_some_and(|value| value.as_array().is_none_or(|lookups| !lookups.is_empty())) {
        return Ok(false);
    }
    let account_keys = message.get("accountKeys").and_then(Value::as_array)
        .ok_or_else(|| ApiError::Unavailable("Solana transaction account keys are invalid".into()))?;
    let account_keys: Vec<&str> = account_keys.iter().map(|value| value.as_str()).collect::<Option<Vec<_>>>()
        .ok_or_else(|| ApiError::Unavailable("Solana transaction account keys must be strings".into()))?;
    let header = message.get("header").and_then(Value::as_object)
    ...
    if required_signatures != 1 || readonly_signed > required_signatures
    ...
    for instruction in &expected.instructions {
        expected_roles.entry(instruction.program_id.as_str()).or_insert((false, false));
        for account in &instruction.accounts {
            let role = expected_roles.entry(account.address.as_str()).or_insert((false, false));
    ...
    let instructions = message.get("instructions").and_then(Value::as_array)
        .ok_or_else(|| ApiError::Unavailable("Solana transaction instructions are invalid".into()))?;
    ...
    for (actual, expected_ix) in instructions.iter().zip(&expected.instructions) {
        let actual = actual.as_object()
            .ok_or_else(|| ApiError::Unavailable("Solana compiled instruction is invalid".into()))?;
        ...
        let actual_accounts = actual.get("accounts").and_then(Value::as_array)
            .ok_or_else(|| ApiError::Unavailable("Solana compiled instruction accounts are invalid".into()))?;
        ...
        let actual_data = actual.get("data").and_then(Value::as_str)
            .ok_or_else(|| ApiError::Unavailable("Solana compiled instruction data is invalid".into()))?;
        let actual_data = bs58::decode(actual_data).into_vec()
            .map_err(|_| ApiError::Unavailable("Solana compiled instruction data is invalid base58".into()))?;
        let expected_data = BASE64_STANDARD.decode(&expected_ix.data_base64)
...
fn usize_field(value: Option<&Value>) -> Result<usize, ApiError> {
    let value = value.and_then(Value::as_u64)
        .ok_or_else(|| ApiError::Unavailable("Solana transaction contains an invalid numeric field".into()))?;
    usize::try_from(value).map_err(|_| ApiError::Unavailable("Solana transaction numeric field is too large".into()))
}
...
mod tests {
    ...
    fn decodes_exact_account_layouts_and_rejects_seed_mismatch() {
        ...
        let station = hex::decode("036b17d1f2e12c4247f8bce6e563a440f277037d812deb33a0f4a13945d898c296").unwrap();
        ...
        assert_eq!(decode_protocol_config(&config, deployment, 7).unwrap().as_slice(), station.as_slice());
        ...
        animal_body.push(8);
        let animal = decode_animal_state(&with_discriminator("AnimalState", animal_body), animal_id, 8).unwrap();
        ...
        binding_body.push(1);
        binding_body.push(6);
        let binding = decode_rfid_binding(&with_discriminator("RfidBinding", binding_body), rfid, 6).unwrap();
    ...
    fn finalized_transaction_requires_exact_requested_signature_and_signature_count() {
        ...
        assert!(!transaction_matches_json(&extra_signature, "fixture-signature", &expected).unwrap());
    }
}
