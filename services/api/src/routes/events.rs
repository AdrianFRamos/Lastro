...
use crate::{
    error::ApiError,
    model::{
        AnimalResponse, ConfirmEventRequest, EventStatus, EventSubmissionResponse,
...
pub async fn transaction_data(
    ...
    Ok(Json(build_transaction_data(
...
pub async fn submit(
    ...
    match record.status.as_str() {
        ...
        _ => {
            return Err(ApiError::Conflict(
                "event is not eligible for transaction submission".into(),
            ));
        }
    }
    if let Some(existing) = record.tx_signature.as_deref() {
        if existing != body.tx_signature {
            return Err(ApiError::Conflict(
                "event already references a different transaction".into(),
            ));
        }
    }
    ...
    if !state
        .rpc
        .transaction_matches_confirmed(&body.tx_signature, &expected_transaction)
        .await?
    {
        return Err(ApiError::Conflict(
...
async fn load_preparable_event(
    ...
    if record.event.deployment_id != state.config.deployment_id {
        return Err(ApiError::Conflict(
            "event belongs to a different deployment".into(),
        ));
    }
    if record.status != "EVIDENCE_ACCEPTED" {
        return Err(ApiError::Conflict(
            "event is not awaiting wallet authorization".into(),
        ));
    }
    Ok(record)
}
...
fn require_same_transaction(
    ...
    if record.tx_signature.as_deref() != Some(transaction_signature) {
        return Err(ApiError::Conflict(
            "event is already associated with a different transaction".into(),
        ));
    }
    Ok(())
}

async fn verify_station_registration(
    state: &AppState,
    record: &EventRecord,
) -> Result<(), ApiError> {
...
async fn verify_canonical_predecessor(
    state: &AppState,
    event: &StationEvent,
) -> Result<(), ApiError> {
    let canonical = state
        .rpc
        .animal_state(event.deployment_id, event.animal_id)
        .await?;
    match event.action {
        Action::Origin => {
            if canonical.is_some() {
                return Err(ApiError::Conflict(
                    "ORIGIN already exists canonically for this AnimalID".into(),
                ));
            }
            if state
                .rpc
                .rfid_binding(event.deployment_id, event.new_rfid_hash)
                .await?
                .is_some()
            {
                return Err(ApiError::Conflict(
                    "RFID has already appeared in canonical history".into(),
                ));
            }
        }
        Action::Transfer => {
        ...
        Action::Reidentify => {
            ...
            {
                return Err(ApiError::Conflict(
                    "new RFID has already appeared in canonical history".into(),
                ));
            }
        }
    }
...
fn verify_predecessor_fields(
    ...
    {
        return Err(ApiError::Conflict(
            "canonical predecessor no longer matches StationEvent".into(),
        ));
    }
    Ok(())
}
...
async fn require_active_binding(
    ...
    let binding = state
        .rpc
        .rfid_binding(event.deployment_id, rfid_hash)
        .await?
    ...
    {
        return Err(ApiError::Conflict(
            "canonical RFID binding is not ACTIVE for this AnimalID".into(),
        ));
    }
    Ok(())
}
...
async fn verify_canonical_terminal(state: &AppState, event: &StationEvent) -> Result<(), ApiError> {
    let canonical = state
        ...
        .await?
        .ok_or_else(|| {
            ApiError::Conflict("finalized transaction did not produce AnimalState".into())
        })?;
    if canonical.animal_id != event.animal_id
    ...
    {
        return Err(ApiError::Conflict(
            "canonical AnimalState does not match event terminal state".into(),
        ));
    }
    ...
    let current = state
        ...
        .await?
        .ok_or_else(|| {
            ApiError::Conflict("finalized transaction did not produce current RFID binding".into())
        })?;
    verify_binding(
        &current,
        event.animal_id,
    ...
    if event.action == Action::Reidentify {
        let old = state
            .rpc
            .rfid_binding(event.deployment_id, event.old_rfid_hash)
            .await?
            .ok_or_else(|| {
                ApiError::Conflict("finalized REIDENTIFY lost the previous RFID binding".into())
            })?;
        verify_binding(
            &old,
            event.animal_id,
...
fn verify_binding(
    ...
    {
        return Err(ApiError::Conflict(
            "canonical RFID binding does not match event terminal state".into(),
        ));
    }
    Ok(())
}
