...
impl AgentConfig {
        ...
        F: FnMut(&str) -> Option<String>,
    {
        ...
        where
            F: FnMut(&str) -> Option<String>,
        {
            let value = lookup(name).ok_or_else(|| AgentError::Config(format!("missing {name}")))?;
        ...
        fn positive_u64(name: &'static str, value: &str) -> Result<u64, AgentError> {
            ...
            if parsed == 0 {
                return Err(AgentError::Config(format!("{name} must be greater than zero")));
            }
            Ok(parsed)
        }

        let api_base_url = required(&mut lookup, "LASTRO_AGENT_API_URL")?;
        let parsed_url = reqwest::Url::parse(&api_base_url)
            .map_err(|_| AgentError::Config("LASTRO_AGENT_API_URL must be an absolute http(s) URL".into()))?;
        if !matches!(parsed_url.scheme(), "http" | "https") {
            return Err(AgentError::Config("LASTRO_AGENT_API_URL must use http or https".into()));
        }
        ...
        let station_baud: u32 = station_baud_raw
            .parse()
            .map_err(|_| AgentError::Config("LASTRO_AGENT_SERIAL_BAUD must be a positive u32".into()))?;
        if station_baud == 0 {
            return Err(AgentError::Config(
        ...
        let pubkey = hex::decode(&pubkey_hex)
            .map_err(|_| AgentError::Config("LASTRO_STATION_PUBKEY_HEX must be lowercase hex".into()))?;
        let station_pubkey33: [u8; 33] = pubkey.try_into().map_err(|_| {
            AgentError::Config("LASTRO_STATION_PUBKEY_HEX must be a 33-byte compressed P-256 public key".into())
        })?;
        lastro_protocol::crypto::derive_station_id(&station_pubkey33)
            .map_err(|_| AgentError::Config("LASTRO_STATION_PUBKEY_HEX is not a valid P-256 public key".into()))?;
