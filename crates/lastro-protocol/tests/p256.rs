...
use lastro_protocol::{ProtocolError, crypto::verify_station_signature};
...
fn valid_compact_low_s_signature_verifies_raw_event() {
    ...
    let signature = common::hex_array::<64>(
        fixture["events"]["origin"]["station_signature_hex"]
            .as_str()
            .unwrap(),
    );
...
fn one_byte_event_change_breaks_signature() {
    ...
    event[200] ^= 1;
    assert_eq!(
        verify_station_signature(&event, &pubkey, &signature),
        Err(ProtocolError::InvalidStationSignature)
    );
}
...
fn wrong_station_key_breaks_signature() {
    ...
    let wrong_pubkey = common::hex_array::<33>(
        "037cf27b188d034f7e8a52380304b51ac3c08969e277f21b35a60b48fc47669978",
    );
    assert_eq!(
        verify_station_signature(&event, &wrong_pubkey, &signature),
        Err(ProtocolError::InvalidStationSignature)
    );
}
...
fn high_s_signature_is_rejected() {
    ...
    let mut signature = common::hex_array::<64>(
        fixture["events"]["origin"]["station_signature_hex"]
            .as_str()
            .unwrap(),
    );
    ...
    assert_eq!(
        verify_station_signature(&event, &pubkey, &signature),
        Err(ProtocolError::HighSSignature)
    );
}
