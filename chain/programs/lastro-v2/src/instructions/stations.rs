...
use crate::{
    constants::{
        CONFIG_V2_SEED, STATION_REGISTRY_SEED, STATION_STATUS_ACTIVE, STATION_STATUS_EXPIRED,
        STATION_STATUS_REVOKED, STATION_V2_SEED, is_valid_station_status,
    },
    error::LastroV2Error,
...
pub fn register_handler(
    ...
    require!(
        matches!(pubkey33[0], 0x02 | 0x03),
        LastroV2Error::InvalidPublicKey
    );
    PublicKey::from_sec1_bytes(&pubkey33).map_err(|_| error!(LastroV2Error::InvalidPublicKey))?;
    require!(
        derive_station_id(&pubkey33) == station_id,
...
pub fn set_status_handler(ctx: Context<SetStationStatus>, status: u8) -> Result<()> {
    require!(
        is_valid_station_status(status),
        LastroV2Error::InvalidStationStatus
    );
    let station = &mut ctx.accounts.station;
    require!(
        station.status != STATION_STATUS_REVOKED,
        LastroV2Error::InvalidStationStatus
    );
    require!(
        status != STATION_STATUS_ACTIVE || station.status != STATION_STATUS_EXPIRED,
        LastroV2Error::InvalidStationStatus
    );
