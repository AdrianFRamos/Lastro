...
fn full_origin_transfer_reidentify_transfer_reaches_expected_terminal_state() {
    ...
    assert_eq!(animal.current_custodian.to_bytes(), h.wallet_c.pubkey().to_bytes());
    ...
    assert_eq!(rfid_binding(&h.svm, &h.deployment_id, &flow.rfid_b).status, RFID_STATUS_ACTIVE);
...
fn replay_of_any_consumed_event_fails() {
    ...
    let snapshot = account_data(&h.svm, &animal_state_pda(&h.deployment_id, &flow.animal_id).0).unwrap();
    h.svm.expire_blockhash();
    // ACTION: Replay ORIGIN after it has already been consumed.
    assert_failure(send_event(&mut h.svm, &flow.origin, &h.station_signing_key, &h.station_pubkey33, &h.wallet_a));
    // ASSERT: The replay fails and state is unchanged.
    assert_eq!(account_data(&h.svm, &animal_state_pda(&h.deployment_id, &flow.animal_id).0).unwrap(), snapshot);

    h.svm.expire_blockhash();
    assert_success(send_event(&mut h.svm, &flow.transfer_ab, &h.station_signing_key, &h.station_pubkey33, &h.wallet_a));
    let snapshot = account_data(&h.svm, &animal_state_pda(&h.deployment_id, &flow.animal_id).0).unwrap();
    h.svm.expire_blockhash();
    assert_failure(send_event(&mut h.svm, &flow.transfer_ab, &h.station_signing_key, &h.station_pubkey33, &h.wallet_a));
    assert_eq!(account_data(&h.svm, &animal_state_pda(&h.deployment_id, &flow.animal_id).0).unwrap(), snapshot);
    ...
    let snapshot = account_data(&h.svm, &animal_state_pda(&h.deployment_id, &flow.animal_id).0).unwrap();
    h.svm.expire_blockhash();
    assert_failure(send_event(&mut h.svm, &flow.reidentify, &h.station_signing_key, &h.station_pubkey33, &h.wallet_b));
    assert_eq!(account_data(&h.svm, &animal_state_pda(&h.deployment_id, &flow.animal_id).0).unwrap(), snapshot);
...
fn stale_revision_after_reidentify_fails() {
    ...
    let result = send_event(&mut h.svm, &stale, &h.station_signing_key, &h.station_pubkey33, &h.wallet_b);
    // ASSERT: It fails and canonical revision remains 2.
    assert_failure_contains(result, "InvalidRevision");
    assert_eq!(animal_state(&h.svm, &h.deployment_id, &flow.animal_id).identity_revision, 2);
...
fn fork_with_valid_station_signature_but_wrong_predecessor_fails() {
    ...
    let result = send_event(&mut h.svm, &fork, &h.station_signing_key, &h.station_pubkey33, &h.wallet_a);
