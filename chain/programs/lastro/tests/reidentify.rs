...
fn reidentify_rejects_new_rfid_already_known() {
    ...
    let before_animal = account_data(
        &h.svm,
        &animal_state_pda(&h.deployment_id, &flow_a.animal_id).0,
    )
    .unwrap();
    let before_old = account_data(
        &h.svm,
        &rfid_binding_pda(&h.deployment_id, &flow_a.rfid_a).0,
    )
    .unwrap();
    h.svm.expire_blockhash();
