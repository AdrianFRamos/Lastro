...
use crate::{
    ...
    verify::{
        event::{ACTION_REIDENTIFY, parse_event, station_id_from_pubkey},
        secp256r1::verify_station_precompile_binding,
    },
};

pub fn handler(ctx: Context<Reidentify>, event: [u8; 276]) -> Result<()> {
    let parsed = parse_event(&event)?;
    require!(
        parsed.action() == ACTION_REIDENTIFY,
    ...
    require!(
        parsed.identity_revision()
            == animal
                .identity_revision
                .checked_add(1)
                .ok_or_else(|| error!(LastroError::InvalidRevision))?,
    ...
    require!(
        Pubkey::new_from_array(parsed.to_custodian()) == current_custodian,
        LastroError::InvalidCustodian
    );
    ...
    require!(
        old_binding.animal_id == animal.animal_id
            && old_binding.rfid_hash == animal.current_rfid_hash,
