...
use crate::{
    ...
    verify::{event::{parse_event, station_id_from_pubkey, ACTION_TRANSFER}, secp256r1::verify_station_precompile_binding},
};

pub fn handler(ctx: Context<'_, '_, '_, '_, Transfer<'_>>, event: [u8; 276]) -> Result<()> {
    let parsed = parse_event(&event)?;
    require!(parsed.action() == ACTION_TRANSFER, LastroError::InvalidEvent);
    ...
    require!(parsed.identity_revision() == animal.identity_revision, LastroError::InvalidRevision);
    require!(parsed.previous_event_hash() == animal.last_event_hash, LastroError::InvalidPredecessor);
    require!(parsed.old_rfid_hash() == animal.current_rfid_hash && parsed.new_rfid_hash() == animal.current_rfid_hash, LastroError::InvalidRfidTransition);
    ...
    require!(destination != current_custodian, LastroError::InvalidCustodian);

    let binding = &ctx.accounts.rfid_binding;
    require!(binding.is_active(), LastroError::RfidBindingMismatch);
    require!(binding.animal_id == animal.animal_id && binding.rfid_hash == animal.current_rfid_hash, LastroError::RfidBindingMismatch);
